...
Live dashboard + automated agent orchestration for the multi-agent sprint coordination system. The dashboard is the single control center for the human — sprint lifecycle, agent management, ticket creation, and real-time monitoring.
...
2. (Optional) Click **Human Control** tab, then **New Ticket** to add tickets to backlog
3. Click **Start Sprint** (green button in header) — all groomed tickets auto-approve to dev-ready, auto-dispatch enables
4. Watch agents work — each agent card shows status, current ticket, and live output log
5. Click any agent card to expand its log preview; click **Stop** / **Start** to control individually
6. Switch to **Agent Logs** tab for real-time orchestrator feed (filterable by agent)
7. Click any ticket card for details, approval actions, or block actions
8. Need to stop one agent? Click **Stop** on its card. Others keep working.
9. Need to halt everything? Click **Halt Sprint**. All agents stop.
10. Sprint finishes (all tickets done) — **Sprint Review** button appears in header
...
│  Sprint Board │ Blackboard │ Agent Logs │ Human Control      │
│  Start Sprint │ Agent cards (Start/Stop) │ New Ticket        │
└─────────┬───────────────────────────────────────────────────┘
          │ SSE + REST API
┌─────────▼───────────────────────────────────────────────────┐
│  server.js (Express)                                         │
...
│  │   ├── Persistent JSONL logs in .agent-board/logs/         │
│  │   └── Events → SSE broadcast to dashboard                 │
│  ├── File watcher (chokidar) on .agent-board/                │
│  └── REST API for human actions + agent control              │
└─────────┬──────────────────────────┬────────────────────────┘
          │ fs.read/write            │ spawns
┌─────────▼──────────────────┐  ┌───▼────────────────────────┐
│  .agent-board/              │  │  Claude Code Agents         │
│  board.json ← tickets      │  │  claude -p (PM)             │
│  blackboard.md ← signals   │  │  claude -p (Dev)            │
│  schema.json ← rules       │  │  claude -p (Reviewer)       │
│  history/ ← audit trail    │  │  claude -p (Test)           │
│  sprints/current.json       │  │  Each runs one cycle, exits │
└─────────────────────────────┘  └────────────────────────────┘
```
...
- **Persistent** — logs are written to `.agent-board/logs/{role}.log` as JSONL and restored on restart
...
|--------|------|-------------|
| GET | `/api/state` | Full board state (includes agents, autoMode, orchestratorLog) |
| GET | `/api/events` | SSE stream — real-time updates (state-update, agent-update, agent-output, orchestrator-log) |
...
| POST | `/api/actions/approve` | Approve ticket `{ticketId}` |
| POST | `/api/actions/block` | Block ticket `{ticketId, reason}` |
| POST | `/api/actions/halt` | Halt sprint `{reason}` — also stops all orchestrator agents |
| POST | `/api/actions/resume` | Resume from halt — also re-enables auto-dispatch |
...
The server uses `chokidar` to watch `.agent-board/` directory. Any file change triggers:
...
The `Orchestrator` class is wired into `server.js` and manages agent lifecycle. It can also be used programmatically:

```javascript
const Orchestrator = require('./orchestrator');
const orch = new Orchestrator('/path/to/your-project');
...
// Logs also persist to .agent-board/logs/dev-agent.log (JSONL format)
...
### How server.js Uses the Orchestrator

- Orchestrator is initialized on server start
- All orchestrator events (`agent-update`, `agent-output`, `log`, `board-change`) are forwarded to SSE clients
- File watcher triggers `orch.evaluateAndDispatch()` when auto-dispatch is on
- `POST /api/actions/halt` calls `orch.stopAll()` to kill agent processes
- `POST /api/actions/resume` calls `orch.startAutoMode()` to re-enable dispatch
- `POST /api/actions/start-sprint` calls `orch.startAutoMode()` after approving tickets
...
- The raw log file: `.agent-board/logs/{role}.log`

### Logs lost after restart

Logs now persist to `.agent-board/logs/` as JSONL files (one JSON object per line). On restart, the last 50 lines per agent are restored. To view raw logs:

```bash
# Tail a specific agent's log
tail -f .agent-board/logs/dev-agent.log | jq .
...
The sprint status persists in `.agent-board/sprints/current.json`. If the server was killed mid-halt, the blackboard may still contain the `[halt]` signal. Either:
- Click **Resume Sprint** in the dashboard
- Or manually remove the `[halt]` section from `.agent-board/blackboard.md` and set `"status": "active"` in `sprints/current.json`
